...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    class CustomTransform:
        def __init__(
            ...
            self.size = size
            self.angle = (
                2 * angle * random.random() - angle
            )  # self.angle in [-angle,angle]
            self.translate = (translate * random.random(), translate * random.random())
            self.scale = (
                2 * scale * random.random() + 1 - scale
            )  # self.scale in [1-scale,1+scale]
            self.shear = (
                2 * shear * random.random() - shear
            )  # self.shear in [-shear,shear]
            self.b_factor = (
                2 * b_factor * random.random() + 1 - b_factor
            )  # self.b_factor in [1-b_factor,1+b_factor]
            self.c_factor = (
                2 * c_factor * random.random() + 1 - c_factor
            )  # self.c_factor in [1-c_factor,1+c_factor]
        ...
        def apply(self, image, mode="image"):
            if mode == "image":
                image = transforms.functional.resize(
                    image, self.size, transforms.InterpolationMode.BILINEAR
                )
                ...
                image = transforms.functional.resize(
                    image, self.size, transforms.InterpolationMode.NEAREST
                )
                image = transforms.functional.affine(
                    image, self.angle, self.translate, self.scale, self.shear
                )
